...
sig_differential <-
    lapply(names(p_value_types), function(pvt){
        ...
        function(x){
            ...
            merged = differential_results[[x]][which(
              abs(differential_results[[x]][,params$differential_fc_column]) > log2(params$differential_min_fold_change) & signif
            ),] %>% mutate(
              Direction = ifelse(
                get(params$differential_fc_column) > 0, "UP", "DOWN"
              ),
              Comparison = contrast_descriptions[[x]]
            )
            )
        }
        )
...
for (i in 1:nrow(contrasts)){
    ...
    for (pvt in names(p_value_types)){
        ...
        # Print merged differential genes table once
        contrast_de_merged <- sig_differential[[pvt]][[i]][["merged"]]
        cols_to_round <- c(params$differential_fc_column, params$differential_pval_column, params$differential_qval_column)
        contrast_de_merged[, cols_to_round] <- signif(contrast_de_merged[, cols_to_round], 8)
        contrast_de_merged <- round_dataframe_columns(contrast_de_merged, digits=params$report_round_digits)
        colnames(contrast_de_merged) <- prettifyVariablename(colnames(contrast_de_merged))
        if (nrow(contrast_de_merged) > 0) {
          print(htmltools::tagList(datatable(
            contrast_de_merged,
            caption = paste("Merged differential genes in", contrast_descriptions[i]),
            rownames = FALSE
          )))
        }
    ...
    for (gene_set_method in possible_gene_set_methods){
        ...
        if (gene_set_method == 'gsea') {
            ...
            for (i in 1:nrow(contrasts)) {
                ...
                # Read and filter target results
                ...
                target_gsea_results$Condition <- contrasts$target[i]

                # Read and filter reference results
                ...
                ref_gsea_results$Condition <- contrasts$reference[i]

                # Combine and print
                combined_gsea_results <- rbind(target_gsea_results, ref_gsea_results)
                print(htmltools::tagList(datatable(
                  combined_gsea_results,
                  caption = paste0("GSEA results for ", contrast_descriptions[i]),
                  rownames = FALSE
                )))
            }
            }
